...
from .MHSA import OverlapPatchEmbed
...
def make_blocks(stage_index,depths,embed_dims,img_size,dpr,extra_token_num=1,attn_embed_dims=[512,1024],meta_dims=[768, ],num_heads=8,mlp_ratio=4.,stage_type='conv'):
    ...
    for block_idx in range(depths[stage_index]):
        ...
        if stage_type == 'conv':
            ...
            blocks.append(MHSABlock(input_dim=in_chans,output_dim=out_chans,
                                    ...
                                    attn_embed_dims=attn_embed_dims,meta_dims=meta_dims,mlp_ratio=mlp_ratio,drop_path=drop_path_rate))
        else:
...
class MetaFG_Meta(nn.Module):
    def __init__(self,img_size=224,in_chans=3, num_classes=1000,
                conv_embed_dims = [64,96,192],attn_embed_dims=[512,1024],
                ...
                add_meta=True,meta_dims=[768, ],mask_prob=1.0,mask_type='linear',
                only_last_cls=False,
        ...
        self.SVT_channel_mixing = Block(dim=256,mlp_ratio=8)
        self.patch_embed = OverlapPatchEmbed(patch_size=3,stride=2,in_chans=256,embed_dim=512)
...
#             assert len(meta_dims)==extra_token_num-1
            ...
            #for ind, freq_dim in enumerate(freq_dims):
            for ind,meta_dim in enumerate(meta_dims):
        ...
        self.stage_1 = nn.ModuleList(make_blocks(1,conv_depths+attn_depths,conv_embed_dims+attn_embed_dims,img_size//4,
                                      dpr=dpr,num_heads=num_heads,extra_token_num=extra_token_num,attn_embed_dims=[512,1024],meta_dims=[768, ],mlp_ratio=mlp_ratio,stage_type='conv'))
        #stage_2
        ...
        self.stage_3 = nn.ModuleList(make_blocks(3,conv_depths+attn_depths,conv_embed_dims+attn_embed_dims,img_size//8,
                                      dpr=dpr,num_heads=num_heads,extra_token_num=extra_token_num,attn_embed_dims=[512,1024],meta_dims=[768, ],mlp_ratio=mlp_ratio,stage_type='mhsa'))
        #stage_4
...
#                 m.bias.data.zero_()
    ...
    def forward_features(self,x,meta=None):
        ...
        if y is not None:
            ...
            # 将 y 投影到不同的目标嵌入维度
            freq_head_1 = getattr(self, "freq_1_head_1")  # 获取映射头 1
            freq_head_2 = getattr(self, "freq_1_head_2")  # 获取映射头 2

            # 映射 y 到 attn_embed_dims[0] 和 attn_embed_dims[1]
            freq_1 = freq_head_1(y)  # 形状: (B, N, attn_embed_dims[0])
            freq_1 = freq_1.reshape(B, -1, self.attn_embed_dims[0])  # 确保维度匹配
            freq_2 = freq_head_2(y)  # 形状: (B, N, attn_embed_dims[1])
            freq_2 = freq_2.reshape(B, -1, self.attn_embed_dims[1])

            extra_tokens_1.append(freq_1)
            extra_tokens_2.append(freq_2)    
        
        # for _ in range(4):
        #     x = self.SVT_channel_mixing(x,H0,W0)
        for ind,blk in enumerate(self.stage_3):         
            if ind==0:  
                #print('self.stage_3--if-----')             
                x = blk(x,H0,W0,extra_tokens_1,frequency,extra_tokens_type="extra_tokens_1",is_stage_3=True)
            else:
                #print('self.stage_3--else-----')                
                x = blk(x,H0,W0)
        if not self.only_last_cls:
            cls_1 = x[:, :1, :]
        ...
        for ind,blk in enumerate(self.stage_4):
            #print('self.stage_4-------')
            if ind==0:
                #print('self.stage_4-if---')               
                x = blk(x,H1,W1,extra_tokens_2,frequency,extra_tokens_type="extra_tokens_2")
            else:       
                x = blk(x,H1,W1)
        cls_2 = x[:, :1, :]
